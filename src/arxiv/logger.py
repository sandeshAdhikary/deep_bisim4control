...
class Logger(object):
    def __init__(self, config, summary_writer=None): 
        ...
        if self._sw is None:
            ...
            if self.sw_type == 'tensorboard':
                ...
                if tags and not isinstance(tags, list):
                    tags = list(tags)
                self._sw = wandb.init(project=project, dir=self._dir, config=tracked_params, tags=tags)
        
        self._logdir = self._sw.dir

        self._train_mg = MetersGroup(
            os.path.join(self._logdir, 'train.log'),
            formating=FORMAT_CONFIG[format_config]['train']
        )
        self._eval_mg = MetersGroup(
            os.path.join(self._logdir, 'eval.log'),
    ...
    def _try_sw_log_agent(self, key, agent, step):
        model_dir = os.path.join(self._dir, 'models')
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        agent.save(self.model_dir, step)

    def _try_sw_log_checkpoint(self, chkpt_name=None):
        if self.sw_type == 'wandb':
            chkpt_path = os.path.join(self._logdir, chkpt_name or 'checkpoint.zip')
            wandb.save(chkpt_path, policy='now')

    def _try_sw_restore_checkpoint(self, chkpt_name=None):
        if self.sw_type == 'wandb':
            wandb.restore(chkpt_name or 'checkpoint.zip', run_path=self._sw.path)
    ...
    def log_checkpoint(self, chkpt_name=None):
        self._try_sw_log_checkpoint(chkpt_name)
    
    def restore_checkpoint(self):
        """
        Restore checkpoint from the summary writer onto a dir
        """
        self._try_sw_restore_checkpoint()
