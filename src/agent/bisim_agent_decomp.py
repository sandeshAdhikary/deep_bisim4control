...
from src.models.sac_ae_decomp import  Actor, Critic, LOG_FREQ
from src.models.transition_model import make_transition_model
from sklearn.cluster import MiniBatchKMeans, KMeans
from src.models.optimizers import PCGradOptimizer

class BisimAgentDecomp(object):
    ...
    def __init__(
        ...
        # decomp_edit: use PCGrad optimizer
        self.critic_optimizer = torch.optim.Adam(
            self.critic.parameters(), lr=critic_lr, betas=(critic_beta, 0.999)
        )
        if self.use_cagrad:
            self.critic_optimizer = PCGradOptimizer(self.critic_optimizer)
    ...
    def update_critic(self, obs, action, vec_rewards, next_obs, not_done, L, step):
        with torch.no_grad():
            _, policy_action, log_pi, _ = self.actor(next_obs)
            # decomp_edit : target_Q1 and target_Q2 are vectors
            target_Q1, target_Q2 = self.critic_target(next_obs, policy_action)
            # decomp_edit: min wrt sum of target_Q vectors; no entropy added since we have a separate Q for entropy
            target_Qs_combined = torch.stack([target_Q1, target_Q2]) # (2, batch_size, num_critics)
            target_V_idxes = torch.argmin(target_Qs_combined.sum(dim=2), dim=0) # min over the sum of Qs. Sum taken over the two Qs for each sub-critic
            # decomp_edit: pick out elements from target_Qs_combined using target_V_idxes
            #             should result in stack([target_Qs_combined[target_v_idxes[b], b, :] for b in range(batch_size)])]])
            target_V_idxes = target_V_idxes.unsqueeze(1)  # Shape: [b, 1]
            batch_size, dim = target_Qs_combined.shape[1:]
            target_Qs = target_Qs_combined[target_V_idxes, torch.arange(batch_size).unsqueeze(1), torch.arange(dim)]
            target_Qs = vec_rewards + (not_done * self.discount * target_Qs)

        # get current Q estimates
        # decomp_edit: current_Q's are vectors
        current_Q1, current_Q2 = self.critic(obs, action, detach_encoder=False)

        critic_loss = 0.5*(current_Q1 - target_Qs)**2 + 0.5*(current_Q2 - target_Qs)**2


        if self.use_cagrad:
            critic_loss = critic_loss.mean(dim=0) # Mean over the batches
            self.critic_optimizer.zero_grad()
            # Project conflicting gradients
            self.critic_optimizer.pc_backward(critic_loss)
        else:
            critic_loss = critic_loss.mean(dim=1).mean() # Mean over the sub-critics, then mean over batches
            self.critic_optimizer.zero_grad()
            # Optimize the critic
    ...
    def update_actor_and_alpha(self, obs, L, step):
        ...
        # decomp_edit: actor_Q1 and actor_Q2 should be vectors
        actor_Q1, actor_Q2 = self.critic(obs, pi, detach_encoder=True)
        # decomp_edit: Min wrt sum(actor_Q1) and sum(actor_Q2)
        actor_Q = torch.min(actor_Q1.sum(dim=1), actor_Q2.sum(dim=1))
    ...
    def update(self, replay_buffer, L, step):
        ...
        if self.vec_reward_from_model:
            ...
            vec_rewards = torch.tensor([x['vec_reward'] for x in info]).to(self.device).to(torch.float32)
            true_reward = vec_rewards.sum(dim=-1)
        ...
        self.update_critic(obs, action, vec_rewards, next_obs, not_done, L, step)
        transition_reward_loss = self.update_transition_reward_model(obs, action, next_obs, true_reward, L, step)
        encoder_loss = self.update_encoder(obs, action, true_reward, L, step)
    ...
    # def update(self, replay_buffer, L, step):
    #     # Decomp_edit: Get vector of rewards from info
    #     obs, action, _, _, next_obs, not_done, info = replay_buffer.sample()
    #     assert info[0].get('vec_reward') is not None, "No vec_reward in sample info"
    #     vec_rewards = torch.tensor([x['vec_reward'] for x in info]).to(self.device).to(torch.float32)
    #     reward = vec_rewards.sum(dim=-1)
    ...
    #     encoder_loss = self.update_encoder(obs, action, reward.unsqueeze(-1), L, step)
    #     total_loss = self.bisim_coef * encoder_loss + transition_reward_loss
    #     self.encoder_optimizer.zero_grad()
    #     self.decoder_optimizer.zero_grad()
    #     total_loss.backward()
