...
class GridworldDataset(Dataset):
    ...
    def __init__(self, mode='train'):
        ...
        self.grid_size = int(np.sqrt(self.data.shape[0]))
        self.bandwidth = 1.0
...
class NeuralEFTrainer(SupervisedTrainer):
    ...
    def _get_kernel(self, X):
        return self.grid_data.get_kernel(X)
    ...
    def log_epoch(self, config):
        ...
        if self.epoch % 1000 == 0:
            with torch.no_grad():
                ...
                pred_eigvecs = self.model(torch.from_numpy(data)).to('cpu').numpy()
                pred_eigvecs = rearrange(pred_eigvecs, '(r1 c1) d -> r1 c1 d', r1=int(np.sqrt(pred_eigvecs.shape[0])))
                ...
                max_viz_dim = min(pred_eigvecs.shape[-1], 6)
                for idx in range(max_viz_dim):
                    pred_eigvec = self.make_heatmap(pred_eigvecs[:,:,idx])
                    true_eigvec = self.make_heatmap(true_eigvecs[:,:,idx])
                    img = make_grid([torch.from_numpy(pred_eigvec), 
...
class NeuralEFModel(EncoderModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            kernel = rearrange(self.kernel, 'r1 c1 r2 c2 -> (r1 c1) (r2 c2)')
            kernel = kernel[idx][:, idx]
