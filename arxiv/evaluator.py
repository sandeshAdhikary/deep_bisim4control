...
#         command = f"""python -m src.evaluator_async \
...
from src.sweep import BisimModel, BisimRLTrainer
import argparse
...
if __name__ == "__main__":
    ...
    evaluator = BisimRLEvaluator(config['evaluator'])

    # Set up the model
    # TODO: Shouldn't need to instantiate trainer
    trainer = BisimRLTrainer(config['trainer'])
    config['model'].update(trainer.env.get_env_shapes())
